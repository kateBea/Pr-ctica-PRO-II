...
void Cjt_usuarios::actualizar_estad_usuario(Cjt_sesiones& se, Cjt_cursos& cu, const string& nombre_us, const string& nombre_prob, int resultado, bool& is)
{
    is = lista[nombre_us].prob_is_solved(nombre_prob);

    if (not is) {
        if (resultado == 1) {
            lista[nombre_us].envio_prob(nombre_prob, resultado, cu, se);
        }
        else {
...
void Cjt_usuarios::acabar_curso_usuario_id(Cjt_cursos& cu, const string& nombre, int& c)
{
    map<string, Usuario>::iterator it = lista.find(nombre);
    c = it->second.consultar_curso();
    bool marca;
    it->second.desuscribir(marca);
    if (marca) {
        cu.modificar_comple_curso_id(c);
        cu.modificar_inscritos_i_esimo(c, 0);
    }
    else cu.modificar_inscritos_i_esimo(c, 0);
